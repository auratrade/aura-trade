...
import {
  ArrowDownToLine, RefreshCw, Eye, CheckCircle2, XCircle,
  Clock, DollarSign, User, Hash, ChevronRight, ChevronLeft,
  AlertCircle, Filter
} from 'lucide-react';
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/deposits?status=${status}&page=${page}`
      );
      if (res.ok) {
    ...
    <div className={styles.page}>
      ...
      <div className={styles.pageHead}>
        <div>
          <h1 className={styles.pageTitle}>
            <ArrowDownToLine size={24} /> طلبات الإيداع
          </h1>
                  ...
                  <tr key={tx.id}>
                    <td>
                      <div className={styles.userCell}>
                        ...
                        <div>
                          <div className={styles.userName}>{tx.user?.username}</div>
                          <div className={styles.userEmail}>{tx.user?.email}</div>
                        </div>
                      </div>
                    </td>
                    <td>
                    ...
                    <td><span className={styles.network}>{tx.network}</span></td>
                    <td>
      ...
      {pagination && pagination.totalPages > 1 && (
        <div className={styles.pagination}>
          ...
          <span>صفحة {page} من {pagination.totalPages}</span>
          <button
            className={styles.pageBtn}
